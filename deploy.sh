...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --max-instances 3 \
  --timeout 120 \
  --set-env-vars "NEO4J_URI=${NEO4J_URI},NEO4J_USER=${NEO4J_USER},NEO4J_PASSWORD=${NEO4J_PASSWORD},GEMINI_API_KEY=${GEMINI_API_KEY},GEMINI_MODEL=${GEMINI_MODEL:-gemini-2.5-flash},API_SECRET_KEY=${API_SECRET_KEY},UI_ACCESS_TOKEN=${UI_ACCESS_TOKEN:-},ADMIN_TOKEN=${ADMIN_TOKEN:-},UCKS_STORAGE=${UCKS_STORAGE:-local},ALLOWED_ORIGINS=${ALLOWED_ORIGINS:-*},RATE_LIMIT_PER_MINUTE=${RATE_LIMIT_PER_MINUTE:-30},CHAT_RATE_LIMIT_PER_MINUTE=${CHAT_RATE_LIMIT_PER_MINUTE:-5},CHAT_DAILY_CAP=${CHAT_DAILY_CAP:-500}"
